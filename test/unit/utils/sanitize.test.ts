...
describe("sanitize", () => {
    ...
    it('should strip out script tags completely', () => {
        const result = sanitize('<b>safe text</b> here <script>alert("bad stuff here")</script>')
    ...
    it('should remove dangerous attributes', () => {
        const result = sanitize('<a onclick="alert(\"bad code here\")">click me</a>', { tags: ['a'] })
        expect(result).not.toContain('bad code')
    })

    it('should remove javascript: links', () => {
        const result = sanitize('<a href="javascript:alert(\"bad code here\")">click me</a>', { tags: ['a'] })
        expect(result).not.toContain('bad code')
    })
})